function spm_extractor2(runIdx)

num=num2str(runIdx+10);
...
jobs{1}.stats{1}.fmri_spec.timing.units = 's1ecs';
...
for nameIdx = 1:size(nameList,1)
    for idx = 1:sizeOnsets
        T_s=cellstr(T.stim(idx,:));
        n_l=cellstr(nameList{nameIdx});
        if( isequal(T_s,n_l) && T.run(idx) == runIdx)
            onsets{nameIdx} = double([onsets{nameIdx} T.onset(idx)]);
            durations{nameIdx} = double([durations{nameIdx} T.duration(idx)]);
        end
    end
end
save ([matPrefix '_' num2str(subject) '_' num2str(runIdx+10)], 'names', 'onsets', 'durations')
